...
const EmailCard: React.FC<Props> = ({ email, onFollowUp, onReply, onDelete, onSimulate }) => {
  ...
  const statusTags = {
    ...
    NEEDS_FOLLOW_UP: { label: 'ALARM!', color: 'text-red-700 font-black' },
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex-1 flex justify-end gap-2">
            {email.status === 'NEEDS_FOLLOW_UP' && (
                <button onClick={onFollowUp} className="win95-button !text-[10px] !py-0 !px-1 group-hover:!text-black">Action</button>
            )}
            <button onClick={() => setIsReplying(!isReplying)} className="win95-button !text-[10px] !py-0 !px-1 group-hover:!text-black">Log Reply</button>
            <button onClick={() => setShowHistory(!showHistory)} className="win95-button !text-[10px] !py-0 !px-1 group-hover:!text-black">?</button>
            <button onClick={onDelete} className="win95-button !text-[10px] !py-0 !px-1 group-hover:!text-black text-red-700 font-bold">X</button>
            <button onClick={onSimulate} className="opacity-0 group-hover:opacity-100 px-1 text-[10px] hover:underline">Fast-fwd</button>
        </div>
      </div>
